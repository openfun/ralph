...
def test_models_edx_ui_seq_next_with_invalid_statement(old, new):
    ...
    invalid_event = json.loads(event.model_dump_json())
    invalid_event["event"]["old"] = old
    invalid_event["event"]["new"] = new

    with pytest.raises(
        ValidationError,
        match="event\n  Value error, event.new - event.old should be equal to 1",
    ):
...
def test_models_edx_ui_seq_prev_with_invalid_statement(old, new):
    ...
    with pytest.raises(
        ValidationError,
        match="event\n  Value error, event.old - event.new should be equal to 1",
    ):
