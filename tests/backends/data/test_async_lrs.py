...
def test_backends_http_async_lrs_instantiation_with_settings():
...
async def test_backends_http_async_lrs_status_with_successful_request(
    httpx_mock: HTTPXMock, async_lrs_backend
):
    """Test the LRS backend status method returns `OK` when the request is
    successful.
    """
    backend: AsyncLRSDataBackend = async_lrs_backend()
...
async def test_backends_http_async_lrs_status_with_request_error(
    httpx_mock: HTTPXMock, async_lrs_backend, caplog
):
...
async def test_backends_http_async_lrs_read_max_statements(
    httpx_mock: HTTPXMock, max_statements: int, async_lrs_backend
):
...
async def test_backends_http_async_lrs_read_without_target(
    greedy: bool, httpx_mock: HTTPXMock, async_lrs_backend
):
    ...
    """
    backend: AsyncLRSDataBackend = async_lrs_backend()
    response = {"statements": [mock_statement() for _ in range(3)]}
    url = "http://fake-lrs.com/xAPI/statements/?limit=500"
...
@pytest.mark.anyio
@pytest.mark.parametrize("greedy", [False, True])
async def test_backends_http_async_lrs_read_without_pagination_with_query(
    httpx_mock: HTTPXMock, greedy: bool, async_lrs_backend
):
...
@pytest.mark.anyio
async def test_backends_http_async_lrs_read_with_pagination(
    httpx_mock: HTTPXMock, async_lrs_backend
):
    """Test the LRS backend `read` method when the request on the target endpoint
...
@pytest.mark.anyio
async def test_backends_http_async_lrs_read_with_pagination_with_query(
    httpx_mock: HTTPXMock, async_lrs_backend
):
    """Test the LRS backend `read` method with a query when the request on the target
...
@pytest.mark.parametrize(
    "chunk_size,simultaneous,max_num_simultaneous,statement_count_logs",
    [
        (None, False, None, [6]),
        (500, False, None, [6]),
        (2, False, None, [6]),
        ...
        (500, True, 2, [6]),
        (2, True, 2, [2, 2, 2]),
        (1, True, 2, [1, 1, 1, 1, 1, 1]),
        (500, True, None, [6]),
        (2, True, None, [6]),
    ],
)
async def test_backends_http_async_lrs_write_without_operation(  # noqa: PLR0913
...
@pytest.mark.anyio
async def test_backends_http_async_lrs_write_without_data(caplog, async_lrs_backend):
...
async def test_backends_http_async_lrs_write_with_unsupported_operation(
    async_lrs_backend, operation_type, caplog, error_msg
):
...
async def test_backends_https_async_lrs_write_with_invalid_parameters(
    ...
    """
    backend: AsyncLRSDataBackend = async_lrs_backend()
    error = "max_num_simultaneous must be a strictly positive integer"
    ...
    error = "max_num_simultaneous is ignored when `simultaneous=False`"
...
@pytest.mark.anyio
async def test_backends_https_async_lrs_write_with_target(
    httpx_mock: HTTPXMock, async_lrs_backend, caplog
):
...
async def test_backends_https_async_lrs_write_with_create_or_index_operation(
    operation_type, httpx_mock: HTTPXMock, async_lrs_backend, caplog
):
