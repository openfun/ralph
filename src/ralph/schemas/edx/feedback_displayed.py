...
from marshmallow.fields import Boolean, Dict, List, Nested, Str
...
class FeedbackDisplayedEventSchema(Schema):
    ...
    hint_label = Str(required=True)
    problem_part_id = Str(required=True)
    trigger_type = Str(
        required=True,
        validate=OneOf(
    ...
    student_answer = List(Str(), required=True)
    choice_all = List(Str())
    correctness = Boolean(required=True)
    question_type = Str(
        required=True,
        validate=OneOf(
            choices=[
    ...
    module_id = Str(required=True)
    hints = List(Dict(keys=Str()))
...
class FeedbackDisplayedSchema(BaseEventSchema):
    ...
    event = Nested(FeedbackDisplayedEventSchema(), required=True)
