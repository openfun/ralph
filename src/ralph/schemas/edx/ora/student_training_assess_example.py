...
from marshmallow.fields import UUID, Dict, Nested, Str
...
class StudentTrainingAssessExampleEventSchema(Schema):
    ...
    corrections = Dict(keys=Str(), values=Str(), required=True)
    options_selected = Dict(keys=Str(), values=Str(), required=True)
...
class StudentTrainingAssessExampleSchema(BaseOraEventSchema):
    ...
    event = Nested(StudentTrainingAssessExampleEventSchema(), required=True)
    event_type = Str(
        required=True,
        validate=Equal(
