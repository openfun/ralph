...
from ralph.schemas.edx.browser import BROWSER_EVENT_TYPE_FIELD, BROWSER_NAME_FIELD
...
def base_browser_event():
    ...
    return lambda **kwargs: event_generator(EventType.BROWSER_BASE, **kwargs)
...
def page_close_event():
    ...
    return lambda **kwargs: event_generator(EventType.BROWSER_PAGE_CLOSE, **kwargs)


@pytest.fixture()
def problem_check_event():
    """Returns a problem_check browser event generator that generates size number of events"""

    return lambda **kwargs: event_generator(EventType.BROWSER_PROBLEM_CHECK, **kwargs)
...
def test_valid_event_type_and_name_should_not_raise_exception(base_browser_event):
    ...
    try:
        for event_type in BROWSER_EVENT_TYPE_FIELD:
            event = base_browser_event(event_type=event_type)
            if event_type != "book":
...
def test_valid_event_type_problem_check(problem_check_event):
    ...
    event = problem_check_event()["event"]
...
def test_valid_event_type_problem_graded_or_similar(event):
    ...
    for event_type in [
        ...
        try:
            ...
            event(event_type, event=event_field)
            event_field = [
                (
...
def test_invalid_event_type_problem_graded_or_similar(event):
    ...
    for event_type in [
        ...
        with pytest.raises(ValidationError) as excinfo:
            event(
                event_type, event=["input_d9fd0115d0be4fe08281f2eaf361744c_2_1=Spain"]
            )
        check_error(excinfo, "Event should be a two-items list")
        with pytest.raises(ValidationError) as excinfo:
            event(
...
def check_invalid_thumbnail_outline_toggled_chapter(
    ...
    with pytest.raises(ValidationError) as excinfo:
        event["chapter"] = "/asset-v1:invalid+course+key+type@asset+block/test.pdf"
        default_browser_event(event_type, event=json.dumps(event))
    check_error(
        excinfo,
        "Event chapter should begin with /asset-v1:org+course+session+type@asset+block/",
...
def test_valid_event_type_textbook_pdf_thumbnail_navigated(default_browser_event):
    ...
    try:
        ...
        pytest.fail(
            "Valid browser event_type textbook.pdf.thumbnail.navigated"
...
def test_valid_event_type_textbook_pdf_zoom_menu_changed(default_browser_event):
    ...
    try:
        ...
        pytest.fail(
            "Valid browser event_type textbook.pdf.zoom.menu.changed "
            "should not raise exceptions"
        )
