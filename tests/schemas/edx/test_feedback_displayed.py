...
def feedback_displayed():
    ...
    return lambda **kwargs: event_generator(EventType.FEEDBACK_DISPLAYED, **kwargs)
...
def test_invalid_context_path_value(feedback_displayed):
    ...
    """
    context = feedback_displayed()["context"]
...
def test_invalid_event_question_type_value(feedback_displayed):
    ...
    is not in the permitted value range
    """
    with pytest.raises(ValidationError) as excinfo:
...
def test_invalid_event_with_choice_all_missing(feedback_displayed):
    ...
    """
    event = feedback_displayed(event_args={"question_type": "optionresponse"})["event"]
