...
from .fields.actor import ActorField
from .fields.context import ContextField


class BaseXapiModel(BaseModelWithConfig):
    """Base model for xAPI statements.

    WARNING: It don't include the required `verb` and `object` fields nor the optional `result`,
    `stored`, `authority`, `version` and `attachments` fields.

    Attributes:
        id (UUID5): Consists of a generated UUID5 from the source event string.
        actor (ActorField): See ActorField.
        context (ContextField): See ContextField.
