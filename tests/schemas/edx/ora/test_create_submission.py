...
def create_submission():
    ...
    return lambda **kwargs: event_generator(EventType.CREATE_SUBMISSION, **kwargs)
...
def test_valid_submission_event_with_submission_type_parameter(create_submission):
    ...
    Check the expected behaviour when the submission parameter is set.
    """
    try:
        event = create_submission(submission_type=CreateSubmissionType.FILE.value)
        assert len(event["event"]["answer"]["parts"]) == 0
        event = create_submission(submission_type=CreateSubmissionType.TEXT.value)
        assert "file_keys" not in event["event"]["answer"]
        assert "files_descriptions" not in event["event"]["answer"]
